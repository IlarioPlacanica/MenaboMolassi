...
function setup() {
 ...
 if(!sheet.getLastRow())sheet.appendRow(['Unita','Stato','Versione','Aggiornato']);
}
function doPost(e) {
 let lock;
 try {
  ...
  rows.forEach(row=>{if(row[0])units[row[0]]={status:row[1]||null,version:Number(row[2])}});
  if(body.action==='write') {
   ...
   sheet.getRange(row<0?sheet.getLastRow()+1:row+2,1,1,4).setValues([[body.id,body.status||'',version,new Date().toISOString()]]);
   SpreadsheetApp.flush();units[body.id]={status:body.status,version};
  }
  return json_({ok:true,units});
