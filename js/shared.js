window.MolassiShared = (() => {
 ...
 async function request(action, fields={}, credential=password()) {
  ...
  const response=await fetch(endpoint,{method:'POST',redirect:'follow',credentials:'omit',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action,password:credential,...fields}),signal:AbortSignal.timeout(20000)});
